...
Real ***f3tensor(nrl,nrh,ncl,nch,ndl,ndh)
...
/* allocate a Real 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
	...
	/* allocate pointers to pointers to rows */
	t=(Real ***) malloc((unsigned int)((nrow+NR_END)*sizeof(Real**)));
	if (!t) nrerror("allocation failure 1 in f3tensor()");
	t += NR_END;
	t -= nrl;

	/* allocate pointers to rows and set pointers to them */
	t[nrl]=(Real **) malloc((unsigned int)((nrow*ncol+NR_END)*sizeof(Real*)));
	if (!t[nrl]) nrerror("allocation failure 2 in f3tensor()");
	t[nrl] += NR_END;
	t[nrl] -= ncl;

	/* allocate rows and set pointers to them */
	t[nrl][ncl]=(Real *) malloc((unsigned int)((nrow*ncol*ndep+NR_END)*sizeof(Real)));
	if (!t[nrl][ncl]) nrerror("allocation failure 3 in f3tensor()");
	t[nrl][ncl] += NR_END;
	t[nrl][ncl] -= ndl;
